...
#For making a map of origin time errors for each earthquake
...
ps=$region\_origin_time_error.ps
Jmap=M7i
R=$coords

errors=$path/Event_origin_time.csv
...
gmt makecpt -Cjet -T-10/10/0.1 -Z > cols.cpt
...
gmt psscale -D1.2i/-0.35i/2.5i/0.1ih -E -Ccols.cpt -Ba5f5g5/:"Epicenter origin time error [s]": -O >> $ps

rm tmp1 tmp2

gmt ps2raster $ps -P -Tf
mv $region\_origin_time_error.pdf $path
rm $ps
evince $path/$region\_origin_time_error.pdf
